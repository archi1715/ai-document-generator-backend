...
presentation_collection = db.get_collection("presentation")
document_history_collection = db.get_collection("document_history")
login_history_collection = db.get_collection("login_history")
...
document_history_collection = None
...
async def initialize_db():
    global client, db, users_collection, documents_collection, user_profiles_collection , presentation_collection , document_history_collection, login_history_collection ,contacts_collection , feedback_collection , subscribers_collection , admin_collection
...
#     global client, db, users_collection, documents_collection, user_profiles_collection ,contacts_collection , feedback_collection , subscribers_collection
    ...
    if client:
        ...
        if "presentation" not in await db.list_collection_names():
            await db.create_collection("presentation")
        if "document history" not in await db.list_collection_names():
            await db.create_collection("document_history")
        if "login history" not in await db.list_collection_names():
        ...
        if "admin" not in await db.list_collection_names():
            await db.create_collection("admin")
         # Get references to collections
        users_collection = db.users
        documents_collection = db.documents
        user_profiles_collection = db.user_profiles
        presentation_collection = db.presentation
        document_history_collection = db.document_history
        ...
        logger.info(f"presentation_collection initialized: {presentation_collection is not None}")
        logger.info(f"document_history_collection initialized: {document_history_collection is not None}")
        logger.info(f"login_history_collection initialized: {login_history_collection is not None}")
        logger.info(f"contacts_collection initialized: {contacts_collection is not None}")
        logger.info(f"feedback_collection initialized: {feedback_collection is not None}")
        logger.info(f"subscribers_collection initialized: {subscribers_collection is not None}")
        logger.info(f"admin_collection initialized: {admin_collection is not None}")
        ...
        presentation_collection = DummyCollection("presentation")
        document_history_collection = DummyCollection("document_history")
        login_history_collection = DummyCollection("login_history")
        ...
        admin_collection = DummyCollection("admin")
        return False
...
def get_presentation_collection():
    return presentation_collection
def get_document_history_collection():
    return document_history_collection
def get_login_history_collection():
